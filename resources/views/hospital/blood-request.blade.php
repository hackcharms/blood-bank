                                    ...
                                    @foreach ($BloodRequests as $request)                                        
                                        <tr role="row" class="odd">
                                            ...
                                            <td>
                                                ...
                                                <form class="hidden" id="accept-{{$request->id}}" action="{{route('hospital.handleRequests')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="request_id" value="{{$request->id}}">
                                                    <input type="hidden" name="status" value="approved">
                                                </form>
                                                <form class="hidden" id="decline-{{$request->id}}" action="{{route('hospital.handleRequests')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="request_id" value="{{$request->id}}">
                                                    <input type="hidden" name="status" value="Decline">
                                                </form>
                                                <div class="d-inline-flex">
                                                    <button onclick="if(confirm('Are you sure to accept ?'))$('#accept-{{$request->id}}').submit()" href="http://" class="btn btn-success m-1">
                                                        Approve
                                                    </button>
                                                    <button onclick="if(confirm('Are You Sure to decline ?'))$('#decline-{{$request->id}}').submit()" class="btn btn-danger m-1">
                                                        Decline
                                                    </button>
                                                </div>
                                                @else
